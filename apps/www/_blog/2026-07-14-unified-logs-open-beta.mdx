...
description: 'One searchable view for logs across every Supabase service, with live tail, filtering, and a timeline.'
author: jordi, kemal_y, steven_eubank
date: '2026-07-14'
categories:
  - product
tags:
  - logs
  - observability
imgSocial: 'unified-logs-open-beta/og.jpg'
imgThumb: 'unified-logs-open-beta/thumb.jpg'
toc_depth: 2
---
...
Open Logs in the dashboard and you see every event as it happens, newest first. Each row shows the timestamp, the service, the method, the path, and the status. Postgres connection events sit next to API requests sit next to Auth events, in the same table.
...
Click a row to open the detail panel. Instead of a wall of JSON, you see the request as it moved through your project: the network timing, the gateway, the database query, the Auth check. Each service adds its own block with the fields that matter for that service. The raw JSON is still there in a second tab when you need it.
