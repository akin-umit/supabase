...
/**
 * Dependency-injection channel for the SQL editor's mutable Monaco state.
 *
 * It owns the shared editor refs and the imperative helpers that read/write
 * them (focus management, editor-SQL extraction, error-highlight decorations).
 * The value holds ONLY stable identities — the `RefObject`s (whose identity
 * never changes) and `useCallback`-stable helpers — memoized once, so the
 * provider never re-renders its consumers. It is not reactive state: anything
 * that needs to trigger a re-render (prompt/diff state) stays in React state
 * and is passed as props, not through this context.
 */
type SQLEditorContextValue = {
  ...
  /**
   * Resolve the SQL to act on from the editor (selection → full value →
   * snippet-content fallback). Returns `undefined` when the editor isn't
   * mounted. The result is an `UntrustedSqlFragment` — promote it to executable
   * via `acceptUntrustedSql` only inside an explicit run/explain user action.
   */
