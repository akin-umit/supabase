...
const ROOT_RESOURCE = /^[A-Za-z0-9_-]{1,64}$/
...
function managementRootUrl(resource: string[]) {
  if (
    resource.length === 0 ||
    resource.length > 3 ||
    resource.some((segment) => !ROOT_RESOURCE.test(segment))
  ) {
    throw new SelfHostedManagementError('Invalid management resource', 400)
  }
  ...
  url.pathname = `/v1/${resource.map(encodeURIComponent).join('/')}`
  url.search = ''
  url.hash = ''
  return url
}
...
export async function requestSelfHostedManagementRoot({
  resource,
  method,
  body,
}: {
  ...
  try {
    const response = await fetch(managementRootUrl(resource), {
      method,
      headers: {
        Accept: 'application/json',
        Authorization: `Bearer ${token}`,
