...
describe('api/self-hosted/auth-config', () => {
  ...
  it('returns a complete enough config for auth settings pages to render', () => {
    ...
    expect(config.EXTERNAL_GITHUB_CLIENT_ID).toBe('')
    expect(config.EXTERNAL_GITHUB_SECRET).toBe('')
    expect(config.EXTERNAL_GITHUB_EMAIL_OPTIONAL).toBe(false)
  ...
  it('maps GoTrue-prefixed self-host env vars into auth subpages', () => {
    ...
    vi.stubEnv('GOTRUE_EXTERNAL_GITHUB_ENABLED', 'true')
    vi.stubEnv('GOTRUE_EXTERNAL_GITHUB_CLIENT_ID', 'github-client-id')
    vi.stubEnv('GOTRUE_EXTERNAL_GITHUB_SECRET', 'github-secret')
    vi.stubEnv('GOTRUE_EXTERNAL_GITHUB_EMAIL_OPTIONAL', 'true')
    vi.stubEnv('GOTRUE_EXTERNAL_GOOGLE_SKIP_NONCE_CHECK', 'true')
    vi.stubEnv('GOTRUE_EXTERNAL_KEYCLOAK_URL', 'https://idp.example.com/realms/main')
    vi.stubEnv('GOTRUE_SMS_PROVIDER', 'twilio')
    vi.stubEnv('GOTRUE_SMS_TWILIO_ACCOUNT_SID', 'twilio-account')
    vi.stubEnv('GOTRUE_SMS_TWILIO_AUTH_TOKEN', 'twilio-token')
    vi.stubEnv('GOTRUE_SMS_TWILIO_MESSAGE_SERVICE_SID', 'twilio-message-service')
    vi.stubEnv('GOTRUE_SAML_ENABLED', 'true')
    vi.stubEnv('GOTRUE_SAML_EXTERNAL_URL', 'https://saml.example.com/metadata')
    vi.stubEnv('GOTRUE_SAML_ALLOW_ENCRYPTED_ASSERTIONS', 'true')
    ...
    expect(config.EXTERNAL_GITHUB_CLIENT_ID).toBe('github-client-id')
    expect(config.EXTERNAL_GITHUB_SECRET).toBe('github-secret')
    expect(config.EXTERNAL_GITHUB_EMAIL_OPTIONAL).toBe(true)
    expect(config.EXTERNAL_GOOGLE_SKIP_NONCE_CHECK).toBe(true)
    expect(config.EXTERNAL_KEYCLOAK_URL).toBe('https://idp.example.com/realms/main')
    expect(config.SMS_PROVIDER).toBe('twilio')
    expect(config.SMS_TWILIO_ACCOUNT_SID).toBe('twilio-account')
    expect(config.SMS_TWILIO_AUTH_TOKEN).toBe('twilio-token')
    expect(config.SMS_TWILIO_MESSAGE_SERVICE_SID).toBe('twilio-message-service')
    expect(config.SAML_ENABLED).toBe(true)
    expect(config.SAML_EXTERNAL_URL).toBe('https://saml.example.com/metadata')
    expect(config.SAML_ALLOW_ENCRYPTED_ASSERTIONS).toBe(true)
  })
