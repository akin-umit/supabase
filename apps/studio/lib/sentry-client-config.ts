// Shared Sentry *client* init options + noise-filtering predicates.
//
// Extracted so BOTH runtimes build the client with identical gating:
//   - Next.js pages router → instrumentation-client.ts (Sentry.init on load)
//   - TanStack Start SPA    → router.tsx (Sentry.init in getRouter, client only)
//
// `@sentry/nextjs` resolves to the real package under Next, and is aliased to
// compat/sentry-nextjs.ts (→ @sentry/tanstackstart-react) under the TanStack
// Vite build. Both surface the same `@sentry/react` client API used here.
...
// Only the `thirdPartyErrorFilterIntegration` factory is read off the passed
// SDK namespace at runtime; typed structurally so either `@sentry/nextjs` or
// `@sentry/tanstackstart-react`'s namespace object can be passed in.
type SentryNamespace = {
  // Loose signature (contravariant param) so either SDK's namespace object is
  // assignable; the real call is made through an `as any` cast below.
  thirdPartyErrorFilterIntegration?: (...args: any[]) => unknown
}

export function buildSentryClientOptions(
  SentryNs: SentryNamespace
  ...
  return {
    ...
    integrations: (() => {
      const thirdPartyErrorFilterIntegration = (SentryNs as any).thirdPartyErrorFilterIntegration
